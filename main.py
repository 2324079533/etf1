...
def generate_professional_report(market_data, etf_data_dict):
    if not LLM_API_KEY:
        return "⚠️ 未配置 LLM_API_KEY，无法生成分析报告。"
    ...
    【输入数据】
    1. 大盘基准(上证指数)近3日数据: {json.dumps(market_data, ensure_ascii=False)}
    ...
    * **💡 策略建议**：给出明确的【买入/持有/观望/减仓】建议及止损止盈参考位。
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.4 # 降低温度，提高分析的严谨性和逻辑性
        )
        return response.choices[0].message.content
    except Exception as e:
        return f"⚠️ 大模型请求失败: {e}"
...
def push_markdown_msg(msg):
    ...
    if FEISHU_WEBHOOK:
        # 飞书如需富文本，推荐使用 webhook 的 interactive 模式，此处使用基础 text 作为兼容
